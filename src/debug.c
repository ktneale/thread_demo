...
#define DEBUG0(module, level, msg) do {   \
    if (debug_ ## module ## _level >= DEBUG_LEVEL_ ## level) fprintf(stderr, #module :  #level : (msg) __func__, __FILE__, __LINE__);                   \
    } while (0)
...
extern app_config_t g_myapp_config;
...
void debug0(const char *module, int level, char *msg, const char *func,
	    const char *file, const int line)
{
    if (!msg) {
	fprintf(stderr,
		"[WARNING] (Error! Bad parameter(s)) %s (%s, line %d)\n",
